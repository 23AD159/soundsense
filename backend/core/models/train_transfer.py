...
def plot_compare_history(history):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(epochs_range, acc, label='TL Training Acc')
    plt.plot(epochs_range, val_acc, label='TL Validation Acc')
    plt.legend(loc='lower right')
    plt.title('Transfer Learning Accuracy')
    
    plt.subplot(1, 2, 2)
    plt.plot(epochs_range, loss, label='TL Training Loss')
    plt.plot(epochs_range, val_loss, label='TL Validation Loss')
    plt.legend(loc='upper right')
    plt.title('Transfer Learning Loss')
    
    plt.savefig(os.path.join(MODELS_DIR, "transfer_history.png"))
